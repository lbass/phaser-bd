...
CommonUnit.prototype = {
  ...
  startAction: function() {
    ...
    var targetOrTargets = this.getTarget(enemyInfoList);
    var target = null;
    me.unit.animations.stop();
    if(targetOrTargets.constructor === Array) {
    ...
    me.game.time.events.add(attackDelay, function() {
      if(me.unitData.type === 'my') {
        fsn.util.showBattleUnitPanel(me.unit, target.unit);
      } else {
        fsn.util.showBattleUnitPanel(target.unit, me.unit);
      }
    }, me);
    ...
    me.game.time.events.add(attackDelay + 200, function() {
      me.beforeAttacking();
      me.startAttacking(movePath, targetOrTargets);
    }, me);
  },
  getTarget: function(enemyInfoList) {
    var index = Math.floor(Math.random() * enemyInfoList.length);
    return enemyInfoList[index];
  },
  startAttacking:function(movePath, targetOrTargets) {
    var me = this;
    me.unit.alpha = 0.3;
