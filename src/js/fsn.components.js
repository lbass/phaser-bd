...
fsn.util.alertStartRound = function(roundCount) {
  ...
  var tween = GAME.add.tween(DISPLAY_ROUND).to({alpha: 1}, 400, Phaser.Easing.Linear.None, true, 0, 0, true);
...
fsn.util.showFadeOutMask= function(duration) {
  GAME.add.tween(GAME_MASK).to({alpha: 0.8}, duration, Phaser.Easing.Linear.None, true, 0, 0, true);
}
...
fsn.util.showBattleUnitPanel = function(left, right) {
  if(left.constructor === Array) {
    for(var i = 0 ; i < left.length ; i++) {
      LEFT_UNIT_BOTTOM_PANELS[left[i]].alpha = 1;
    }
  } else {
    LEFT_UNIT_BOTTOM_PANELS[left].alpha = 1;
  }
